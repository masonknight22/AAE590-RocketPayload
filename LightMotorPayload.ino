...
void loop(){
  ...
  int trendKey = luxAnalysisIncluding(luxVal);
  Serial.print("Trend Key: ");
  Serial.println(trendKey);

  if (PayloadState == LoadingPayload && luxVal < LowLux && trendKey == 2) {
    PayloadState = WaitingPayload;
    Serial.println("Waiting for Deployment");
  }
